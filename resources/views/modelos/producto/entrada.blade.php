    ...
    <script>
        $(document).ready(function() {
            ...
            $('#origen_stock_id').on('change', function() {
                ...
                if (origenVal == '0') {
                    $('#destino_stock_id').val('4');
                } else {
                    if (origenVal == $('#destino_stock_id').val()) {
                        var origenVal = $('#origen_stock_id').val();
                        var found = false;
                        $('#destino_stock_id option').each(function() {
                            var currentVal = $(this).val();
                            if (
                                currentVal !== origenVal &&
                                currentVal !== '' &&
                                currentVal !== '4' &&
                                !found
                            ) {
                                $('#destino_stock_id').val(currentVal);
                                found = true;
                            }
                        });
                    }
            ...
            $('#destino_stock_id').on('change', function() {
                ...
                if (destinoVal == '4') {
                    $('#origen_stock_id').val('0');
                } else {
                    if (destinoVal == $('#origen_stock_id').val()) {
                        var destinoVal = $('#destino_stock_id').val();
                        var found = false;
                        $('#origen_stock_id option').each(function() {
                            var currentVal = $(this).val();
                            if (
                                currentVal !== destinoVal &&
                                currentVal !== '' &&
                                currentVal !== '0' &&
                                !found
                            ) {
                                $('#origen_stock_id').val(currentVal);
                                found = true;
                            }
                        });
                    }
