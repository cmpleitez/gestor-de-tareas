...
<div class="container-fluid" data-risk-distribution="{{ trim(json_encode($risk_distribution ?? $defaultRiskData)) }}"
    ...
    <div class="row mb-4">
        <!-- Eventos Recientes -->
        <div class="col-xl-6">
            <x-security.recent-events :events="$recent_events ?? collect()" title="Eventos Recientes"
                icon="fas fa-clock" />
        </div>

        <!-- Top IPs Sospechosas -->
        <div class="col-xl-6">
            <x-security.suspicious-ips :ips="$top_suspicious_ips ?? collect()" title="Top 10 IPs Sospechosas"
                icon="fas fa-list-ol" />
        </div>
    </div>
</div>
