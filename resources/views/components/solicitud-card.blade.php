...
<div class="solicitud-card" data-id="{{ $solicitud['recepcion_id'] }}"
    data-atencion-id="{{ $solicitud['atencion_id'] }}" data-estado-id="{{ $solicitud['estado_id'] }}"
    data-fecha="{{ $solicitud['created_at'] }}" style="border-left-color: {{ $borderColor }};">
    ...
    <div class="progress-divider" data-atencion-id="{{ $solicitud['atencion_id'] }}"
        data-avance="{{ $solicitud['porcentaje_progreso'] }}">
    </div>

    <div class="users-container"
        style="display: flex; align-items: center; justify-content: end; margin-top: 8px; padding-top: 6px;">
        @foreach ($solicitud['users'] as $user)
        <div style="margin: 0;" data-toggle="popover" data-title="{{ $user['name'] }}" data-content="<span class='badge badge-pill {{ $badgeColor }}'>{{ $user['recepcion_role_name'] }}</span> 
                <span class='badge badge-pill {{ $badgeColor }}'>{{ $user['area_name'] }}</span>" data-trigger="hover"
            data-placement="top">
