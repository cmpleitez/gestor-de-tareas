...
class KeyMaker {
    public function generate(string $model, int $solicitud_id): string {
        ...
        $solicitud_id_padded = str_pad($solicitud_id, 4, '0', STR_PAD_LEFT); // 4 dígitos
        ...
        // Crear instancia del modelo dinámicamente
        ...
        if ($maxCorrelativo) {
            $correlativo_actual = (int)substr($maxCorrelativo, -4); // Extraer los últimos 4 dígitos (correlativo)
            $nuevo_correlativo = str_pad($correlativo_actual + 1, 4, '0', STR_PAD_LEFT);
        } else {
            $nuevo_correlativo = '0001'; // Primer correlativo
        }
