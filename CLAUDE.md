...
- **El rol `admin` es excluyente, no exclusivo.** Puede haber **varios administradores** (lo exige la auditoría), pero: (a) *Acceso:* quien es `admin` **no puede tener ningún otro rol Spatie** — así Spatie no mezcla los accesos que viven dentro del gestor de tareas; (b) *Gestión:* `role_id` solo admite papeles del flujo (cliente, receptor, operador), y al marcar `admin` el servidor **impone `role_id = 1`**, que es el marcador de "no participa en el flujo". Sin esa imposición un administrador entraría a los sorteos de participantes (`TiendaController:313`, `RecepcionController:125`) sin tener `ver-solicitudes`, y la solicitud nacería muerta. Blindado en `UserController::rolesEdit/rolesUpdate` y en `roles-edit.blade.php` (checkboxes excluyentes + `<select>` inhabilitado). - **La cuenta `admin` (`username = 'admin'`) es la cuenta de rescate (break-glass)** por si los administradores titular y suplente pierden su acceso. De los 7 métodos de `/accounts` que la tocan, **5 la rechazan para cualquier actor** (`rolesUpdate`, `equiposUpdate`, `tareasUpdate`, `activate`, `destroy`) con el guard de una sola condición `$user->username === 'admin'`; solo se modifican por seeder o BD. **Dos excepciones deliberadas (2026-07-28):** (a) `resetDosFactores` — cualquier administrador puede restablecerle el doble factor, para que un extravío de teléfono + códigos no obligue a bajar a consola; (b) `update` — la cuenta puede editar sus propios datos, guard `$user->username === 'admin' && auth()->user()->username !== 'admin'`, con el bloqueo espejado en la vista para que la UI no prometa lo que el servidor niega. Ver Memoria. **Su fila ya NO se oculta en `index()`** (revertido el 2026-07-28): sin verla, el rescate mutuo era inalcanzable. La protección nunca estuvo en ese filtro sino en el endpoint, que es donde sigue.
...
**⚠️ SUPERADO el 2026-07-28 — el filtro fue eliminado; `index()` hoy es un `User::with(...)->get()` sin `where`.** Razón: al habilitarse el rescate mutuo del doble factor entre administradores, ocultar la fila de `admin` volvía inalcanzable el único control que se quería exponer. Se conserva el registro de abajo porque el razonamiento sigue vigente (el filtro era aislamiento de navegación, nunca autorización por objeto) y porque documenta el bug de `main_role`.

**✅ RESUELTO el 2026-07-25 — regla de visibilidad de `/accounts` (`UserController::index()`):** el filtro pasó por dos iteraciones y quedó así:
...
### 2026-07-28 — 2FA en autoservicio desde el perfil del usuario

**Diagnóstico previo:** el backend de 2FA ya estaba completo (feature de Fortify con `confirm` + `confirmPassword`, trait `TwoFactorAuthenticatable`, migración `030_`, `pragmarx/google2fa` + `bacon/bacon-qr-code`, reto de login con limiter, `lang/es.json` traducido). Lo que faltaba era **presentación**: `/user/profile` reventaba porque `App\View\Components\AppLayout` renderiza `view('layouts.app')` y en `resources/views/layouts/` solo existe `guest.blade.php`; además las vistas Jetstream son Tailwind y el proyecto no tiene Tailwind. El restablecimiento de contraseña por correo ya existía y no forma parte del 2FA.

**Lo aplicado (5 cambios):**
1. **Ruta del perfil reubicada.** `Jetstream::ignoreRoutes()` en `JetstreamServiceProvider::register()` (mismo patrón y misma razón que `Fortify::ignoreRoutes()`: el provider de vendor registra rutas en `boot()`). `profile.show` se declara en `routes/web.php` dentro del grupo ENROLADAS, con lo que hereda `auth:sanctum` + `auth_session` + **`verified`** — el vendor la registraba sin `verified`, era la única ruta del sistema sin él. No se pierde nada: teams, api tokens y terms estaban desactivados.
2. **`profile/show.blade.php`** pasó de `<x-app-layout>` a `@extends('dashboard')` + `@section('contenedor')`, con solo el bloque de 2FA. **No se creó `layouts/app.blade.php`**; `AppLayout.php` ya estaba muerto y sigue muerto (decisión del usuario: "deja muerto quien ya está muerto"). Las otras 4 vistas Jetstream del perfil quedan inertes, sin ruta que las alcance.
3. **`profile/two-factor-authentication-form.blade.php`** reescrito en Bootstrap 4/Vuexy, **sin ningún componente `x-*`** (así no se tocan los componentes compartidos). El componente Livewire de vendor no se toca. Dos piezas propias: (a) el modal de confirmación de contraseña es un modal de Bootstrap gobernado por `@if($confirmingPassword)`, sin la API JS de Bootstrap ni Alpine; (b) un bloque `@script` escucha `Livewire.on('password-confirmed', ({id}) => $wire.call(id))` — se aprovecha que el `confirmableId` del trait `ConfirmsPasswords` puede ser directamente **el nombre del método a ejecutar**, lo que sustituye el baile de `md5()` + eventos Alpine de Jetstream. Textos en español literal.
4. **Ítem *Mi perfil*** en el dropdown del avatar (`dashboard.blade.php`), sin `@can`: todo usuario autenticado gestiona su propio 2FA.
5. **`auth/two-factor-challenge.blade.php`** restilado a página Vuexy autocontenida con el molde de `login.blade.php` (mismo camino que ya había recorrido `verify-email.blade.php`). El toggle código-OTP ↔ código-de-recuperación pasó de Alpine a jQuery y **vacía el campo que oculta**, porque `TwoFactorAuthenticatedSessionController::store()` decide la vía por la presencia de `recovery_code`. Los errores se muestran con toastr, igual que el login; si falló la vía de recuperación, la pantalla vuelve a abrirse en esa vía (`$errors->has('recovery_code')`). Fortify manda ambas claves de error (`code` o `recovery_code`, ver `FailedTwoFactorLoginResponse`).

**Decisión sobre la cuenta `admin` (revisada en la conversación):** **sí puede activar 2FA**, sin guard en el paso 2. El razonamiento inicial que se lo prohibía estaba mal planteado: el break-glass real de un sistema no es una cuenta, es el acceso a la base de datos, y esa vía nunca se pierde. Prohibirle el 2FA dejaba la cuenta más golosa protegida solo con contraseña, que es justo lo contrario del propósito. Lo que **sí** lleva guard es el paso 4: nadie puede restablecerle el 2FA desde `/accounts`.

**Paso 4 — `UserController::resetDosFactores()` + `POST accounts/reset-2fa/{user}` (`can:editar`).** Es la segunda capa estándar de recuperación (la primera son los 8 códigos de recuperación, autoservicio). Reutiliza la acción `DisableTwoFactorAuthentication` de Fortify en vez de escribir el `UPDATE` a mano. Guard de la cuenta de sistema + `Log::error`; rechaza también a quien no tenga 2FA habilitado; deja `Log::warning` de auditoría al ejecutarse, porque la acción retira un factor de seguridad de otra cuenta. Botón escudo en el tablero de control de `modelos/user/index.blade.php`, en tres estados: bloqueado para `admin`, gris si el usuario no tiene 2FA, activo (form POST) si lo tiene. **Ojo:** ese botón condiciona por `$user->username === 'admin'`, no por el `$isAdminUser` de la vista (que es por rol) — el guard del controlador es por username y ambos deben coincidir.

**Cómo funciona la recuperación (verificado en vendor):** al habilitar se generan **8** códigos `Str::random(10).'-'.Str::random(10)`, encriptados. Al usar uno, `replaceRecoveryCode()` lo **sustituye por uno nuevo** (siempre hay 8, pero la lista cambia → hay que volver a consultarla en el perfil). *Regenerar* invalida los 8 de golpe. Reinstalar Google Authenticator en otro teléfono recupera los códigos OTP **solo si la sincronización en la nube estaba activa antes** de perder el equipo; el authenticator nunca regenera los códigos de recuperación de Fortify.

**Verificado en navegador (mismo día, autorizado):** ciclo completo con la cuenta `admin` — habilitar → modal de clave → QR 192×192 + clave de configuración → OTP → 8 códigos de recuperación → mostrar → regenerar (los 8 cambian) → deshabilitar. Login con OTP ✅; login con código inválido ✅ (toast en español); toggle *¿Perdió su teléfono?* ✅ (vacía el campo oculto); login con código de recuperación ✅ (el usado fue reemplazado, siguen 8). `/accounts`: escudo en sus tres estados ✅; restablecimiento real ✅ con `Log::warning`; POST forzado sobre `admin` rechazado ✅ con `Log::error` (esa prueba corresponde al guard que después se retiró, ver abajo). `verified` en `profile.show` ✅ (desverificado redirige a `/email/verify`). `php artisan test` 29 passed / 3 skipped. Base de datos devuelta a su estado inicial en cada corrida.

**Trampa del enrolamiento (comportamiento de vendor, no bug):** el `mount()` de `TwoFactorAuthenticationForm` deshabilita el 2FA si encuentra un secreto con `two_factor_confirmed_at` nulo. Consecuencia: **si el usuario recarga o abandona la pantalla del QR antes de confirmar el OTP, el secreto se borra y el QR escaneado queda inservible.** Hay que completar el enrolamiento de una sentada. Es la causa más probable de un `DecryptException: The payload is invalid` al pedir el OTP por consola: la columna quedó en `null`.

**⚠️ El reloj del equipo de desarrollo está adelantado ~162 s** respecto a la hora real, porque sincroniza contra el controlador de dominio `AMSS-DC-02.amss.gob.sv` (192.168.20.26), que es el desfasado. `w32tm /resync` no lo corrige: obedece a esa fuente. Con ventana TOTP de ±30 s, **ningún código de Google Authenticator entra**. Para desarrollar, generar el OTP desde consola (usa el mismo reloj, así que siempre coincide):

```bash
php artisan tinker --execute='$u=App\Models\User::where("username","admin")->first(); echo app(PragmaRX\Google2FA\Google2FA::class)->getCurrentOtp(decrypt($u->two_factor_secret));'
```

Ojo: el método vive en `PragmaRX\Google2FA\Google2FA`, **no** en `Laravel\Fortify\TwoFactorAuthenticationProvider` (que no tiene `getCurrentOtp`). El `'window'` comentado en `config/fortify.php` no sirve: esta versión de Fortify lo lee de `config('fortify-options.two-factor-authentication.window')`, archivo que el proyecto no tiene. **Verificar la hora del servidor antes de activar 2FA en producción:** un reloj corrido deja fuera a todos y el mensaje de error no apunta a la causa.

**Rescate por consola de cualquier cuenta (ambas vías probadas):**

```bash
php artisan tinker --execute='app(Laravel\Fortify\Actions\DisableTwoFactorAuthentication::class)(App\Models\User::where("username","admin")->firstOrFail());'
```

```sql
UPDATE users SET two_factor_secret=NULL, two_factor_recovery_codes=NULL, two_factor_confirmed_at=NULL WHERE username='admin';
```

Tras cualquiera de las dos, la cuenta entra con usuario y clave sin reto. **No existe re-enrolamiento forzoso:** no hay middleware `Requiere2FA` ni ruta `two-factor.setup` en este proyecto (barrido con cero coincidencias fuera de `vendor/`). El 2FA es voluntario, así que volver a habilitarlo queda a criterio del usuario desde su perfil.

### 2026-07-28 (segunda vuelta) — Los administradores se rescatan entre sí

**Decisión del usuario, tras sopesar el riesgo:** rescatar una cuenta de administrador no se considera dañino, así que **cualquier usuario con rol `admin` puede restablecer el doble factor de cualquier otro, incluida la cuenta de sistema `admin`**. Se retiró el guard de `resetDosFactores` y la rama bloqueada del escudo en la vista. Riesgo aceptado explícitamente: quien robe una sesión de administrador puede quitarle el segundo factor a la cuenta de rescate. Contrapeso: el `Log::warning` de auditoría de cada restablecimiento.

**Efecto dominó que obligó a revertir el filtro de `index()`:** con el guard retirado, el escudo seguía siendo inalcanzable porque la fila de `admin` estaba oculta para los demás. Verificado antes de tocar nada: un segundo administrador veía solo `cpleitez, hseldom, maragon`. Se eliminó el `where` completo. No debilita nada: `/accounts` ya exige `role:admin` y los otros seis controles de esa fila conservan su guard en el endpoint.

**Probado con dos administradores reales** (rol `admin` prestado a `cpleitez` y clave temporal, ambos revertidos al terminar): `cpleitez` ve la fila del Administrador con el escudo activo → restablece → `admin` entra sin reto → log `[Usuario: Carlos Pleitez] Restableció el doble factor de "Administrador"`. Suite 29 passed / 3 skipped; BD y roles devueltos a su estado original.

**Lo que NO se hizo, y por qué:** el usuario pidió inicialmente un botón para que el `admin` se liberara a sí mismo desde `/accounts`. Se descartó tras señalarle que ya existe —*Mi perfil* → **Deshabilitar**, sin guard— y que en `/accounts` sería inútil justo en el escenario que lo motiva: si perdió el teléfono no puede iniciar sesión, luego nunca llegaría a esa pantalla.

**Icono del control (corregido el mismo día):** `bxs-shield-x` **no existe** en la versión de boxicons del proyecto (`public/app-assets/fonts/boxicons/`) — medido: ancho 0 px, sin glifo, por eso el control se veía vacío. Los únicos escudos disponibles son `bx-shield`, `bx-shield-alt`, `bx-shield-alt-2`, `bxs-shield`, `bxs-shield-alt-2`. Quedó: **`bxs-shield`** (sólido) cuando la cuenta tiene doble factor y el control lo retira; **`bx-shield`** (contorno, gris) cuando no lo tiene. Sólido = protegido, contorno = desprotegido, así el icono comunica el estado sin leer el tooltip. Verificados los otros 9 iconos de esa vista más el `bx-user` del menú del avatar: todos existen. **Al agregar un icono, comprobarlo primero** contra `boxicons.css`; una clase inexistente no falla, simplemente no pinta nada.

**Editar de la cuenta de sistema (2026-07-28):** `update()` pasó del guard absoluto a `$user->username === 'admin' && auth()->user()->username !== 'admin'` — la cuenta `admin` puede editar sus propios datos, ningún otro actor puede tocarlos. La vista espeja la condición: control gris con candado para los demás, activo para ella misma. Probado con dos administradores: el segundo ve el botón bloqueado, su PUT forzado se rechaza con `Log::error`, y el PUT de la propia cuenta se acepta.

**🐞 Bug encontrado y corregido el mismo día — el botón *Otorgar* de `/accounts/roles-edit/{user}` no hacía nada al elegir el rol `admin`.** Silencio absoluto: sin toast, sin cambio en BD y **sin una línea en el log**. Causa: `excluirAdministrador()` vacía el `<select name="role_id">`, que en el marcado llevaba `required`; **un `<select required>` que queda vacío veta el envío**, y como además queda `disabled` el navegador no puede señalarlo con su globo de validación → no envía y no avisa. La petición nunca salía del navegador, de ahí la ausencia de log. Diagnóstico por descarte midiendo si el evento `submit` llegaba a dispararse; el discriminante final fue que con el select **con valor** sí enviaba y **vacío** no, independientemente de `disabled` y de quitar `required` por JS.

Corrección en dos partes, ambas necesarias: (a) `roles-edit.blade.php` — eliminado `required` del select; la obligatoriedad ya vivía donde corresponde, en `Rule::requiredIf(! $esAdministrador)`, porque depende de si se marcó admin; (b) `rolesUpdate()` — mensajes de validación en español y un `catch (ValidationException)` **antes** del genérico, porque `ValidationException extends Exception` y el catch de abajo la convertía en el opaco *"The given data was invalid"*. Sin (b), quitar el `required` solo habría cambiado un silencio por un mensaje inútil.

**Lección aplicable al resto del proyecto:** no poner `required` en HTML sobre un campo cuya obligatoriedad es **condicional en el servidor**. El navegador no conoce la condición y bloquea sin diagnóstico. Verificados los tres escenarios: admin (guarda con `role_id=1` impuesto), rol normal con gestión (guarda), rol normal sin gestión (*"Debe seleccionar el rol para la gestión de tareas."*, sin tocar la BD).

**Matriz de recuperación resultante:**

| Situación | Vía |
|---|---|
| Admin dentro, quiere quitarse el 2FA | *Mi perfil* → Deshabilitar (pide clave) |
| Perdió el teléfono, tiene códigos | Código de recuperación en el login |
| Perdió teléfono **y** códigos | Otro administrador: escudo en `/accounts` |
| No queda ningún administrador operativo | Consola: tinker o SQL |
...
**Propagado con `sync-reglas.ps1 -Todos` (v:`012427c`)** a los 10 proyectos con git bajo `www/`. ⚠️ Efecto colateral: el sync **creó `CLAUDE.md` desde plantilla** en 8 proyectos que no lo tenían (alerta-bus, asiste, binder, espacios-municipales, nextcapital, py-base, py-react-base, react-base). Verificado: sello `012427c` coincide con el hash del maestro en los 5 proyectos muestreados.

**Commits + push (2026-08-06):** 11 repos commiteados con mensaje `sync` (convención de los repos). Pusheados a GitHub 7: estandares, gestor-de-tareas, binder, espacios-municipales, demo-laravel-12 (merge limpio), alerta-bus y py-react-base (conflicto add/add en `CLAUDE.md` resuelto: bloque global nuevo + Memoria del otro equipo conservada; el otro equipo había corrido sync con el script viejo el 09/07 y 22/07). **Sin remoto en GitHub, quedan solo locales: asiste, nextcapital, py-base, react-base** — pendiente de revisión del usuario, que cree que debe eliminarlos si no tienen reflejo remoto.
