...
- **No modificar código fuera de lo pedido.** Pedir confirmación antes de crear/eliminar archivos, métodos o funciones, y antes de cambiar algo no solicitado expresamente.
- **No probar de forma proactiva.** Solo iniciar verificación cuando el usuario diga `pruebalo` (workflow en `.agent/workflows/pruebalo.md`) o `buscar-error` (`.agent/workflows/buscar-error.md`). Cuando se pida `pruebalo`, usar la pestaña del navegador ya activa — nunca abrir una nueva — y consultar la ruta sesionada antes de actuar.
- **Sin datos hardcodeados** ni en backend ni en frontend: los datos de prueba siempre provienen de seeders/DB.
- **Comentarios** a la derecha de la primera línea del bloque.
- **Debugging**: `console.log` en frontend, `Log::info()` en controladores. Si hay `dd()`/`Return` de depuración, no son errores; pedir que se retiren al probar.
- Trabajo paso a paso (máximo 3 pasos por respuesta) y dar nombres exactos de opciones y su ubicación en pantalla.
- Reutilizar servicios y estilos existentes antes de duplicar.
